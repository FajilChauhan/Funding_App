  ...
  useEffect(() => {
    const fetchUsers = async () => {
      ...
      if (data && Array.isArray(data.users)) {
        const slicedUsers = data.users.slice(0, 2);
        setUsers(slicedUsers);

        const top5 = data.users
          .filter((user) => user.type === "donater")
    ...
    <div className="bg-gray-100 min-h-screen text-black">
      ...
      <div className="flex flex-col items-center justify-center gap-4 py-8 px-4 md:px-6 text-center">
        <div className="font-bold text-3xl md:text-5xl flex items-center justify-center">
          ...
          <img src="/book.webp" width={55} height={55} className="w-[45px] md:w-[55px]" alt="Logo" />
        </div>
      </div>
          ...
          {users.map((user) => (
            ...
            >
              <img
                src={
                  user.profilepic ||
                  "https://insidetime.org/wp-content/uploads/2021/10/Handing-in-books.jpg"
                }
                alt="User"
                width={80}
                height={80}
            ...
            >
              ...
              <div className="flex flex-col text-center sm:text-left">
                ...
                <span className="text-purple-700">
                  Total Donated: ₹{(user.totalAmount || 0) / 100}
                </span>
              </div>
            </div>
          ))}
      ...
      <div className="w-full flex flex-col items-center justify-center px-4 mb-16">
        ...
        <div className="w-full max-w-5xl bg-white p-6 rounded-lg shadow-md border border-gray-200 text-base">
          <p className="mb-4">
            <strong>GetMe-A-Book</strong> is a crowdfunding platform dedicated to helping
            people access the books they need for education.
          </p>
          <p className="mb-4">
            If you are part of an NGO, you can also create an account and raise funds as a
            receiver. Or, if you want to donate as an organization, you are welcome to be a
            supporter.
          </p>
          <p className="mb-4">
            Our platform is designed for people who want to help those around them. If you
            know someone nearby who needs help buying a book but you do not have enough funds
            yourself, you can also raise money on their behalf using our app.
          </p>
          <p className="mb-4">
            We believe every donation makes a difference. That is why we have built a{" "}
            <strong>leaderboard</strong> that highlights the best donors of the month.
          </p>
          <p>
            To start receiving donations, you will need to set up a Razorpay account with your{" "}
            <strong>Razorpay ID</strong> and <strong>Razorpay Secret Key</strong>.
          </p>
      ...
      <div className="w-full flex flex-col items-center justify-center px-4 mb-16 text-black">
        ...
        <div className="w-full max-w-xl bg-white p-6 rounded-lg shadow-md border border-gray-200 text-center">
          <p className="mb-2">
            📞 Get-Me-A-Book Help-Line:{" "}
            <span className="text-blue-900 font-semibold">8866430415</span>
          </p>
          <p className="mb-2">
            📧 Email:{" "}
            <span className="text-blue-900 font-semibold">support@getmeabook.in</span>
          </p>
          <p>
            📱 Instagram:{" "}
            <span className="text-blue-900 font-semibold">getmeabook_27.12</span>
          </p>
        </div>
      </div>
    </div>
