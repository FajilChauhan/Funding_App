...
useEffect(() => { 
  const fetchUsers = async () => { 
    ...
    if (data && Array.isArray(data.users)) { 
      ...
      const top5 = data.users.filter((user) => 
        user.type === "donater")
...
return ( 
  ...
  <Image src="/book.webp" width={55} height={55} className="w-[45px] md:w-[55px]" alt="Logo" /> 
  </div> 
  </div>
...
<div className="w-full py-6 px-4">
  ...
  <div className="w-full flex flex-col items-center justify-center px-4 mb-16">
    ...
    <div className="w-full max-w-5xl bg-white p-6 rounded-lg shadow-md border border-gray-200 text-base">
      ...
      <p className="mb-4">
        Our platform is designed for people who want to help those around them. If you know someone nearby who needs help buying a book but you do not have enough funds yourself, you can also raise money on their behalf using our app.
      </p>
      <p className="mb-4">
        We believe every donation makes a difference. That is why we have built a <strong>leaderboard</strong> that highlights the best donors of the month.
      </p>
      <p>
        To start receiving donations, you will need to set up a Razorpay account with your <strong>Razorpay ID</strong> and <strong>Razorpay Secret Key</strong>.
      </p>
